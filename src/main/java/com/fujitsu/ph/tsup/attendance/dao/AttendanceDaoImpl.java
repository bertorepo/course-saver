...
public class AttendanceDaoImpl implements AttendanceDao {
	...
	public Set<CourseSchedule> findAllScheduledCourses(ZonedDateTime fromDateTime, ZonedDateTime toDateTime,
			Long instructorId) {
		String sql = "SELECT " + "CSCHED.ID AS ID, " + "CSCHED.COURSE_ID AS COURSE_ID," + "C.NAME AS COURSE_NAME,"
				+ "CSCHED.INSTRUCTOR_ID AS INSTRUCTOR_ID," + "E.LAST_NAME AS INSTRUCTOR_LAST_NAME,"
				+ "E.FIRST_NAME AS INSTRUCTOR_FIRST_NAME," + "CSCHED.VENUE_ID AS VENUE_ID," + "V.NAME AS VENUE_NAME,"
				+ "CSCHED.MIN_REQUIRED AS MIN_REQUIRED," + "CSCHED.MAX_ALLOWED AS MAX_ALLOWED," + "("
				+ "SELECT COUNT(PARTICIPANT_ID) " + "    FROM COURSE_PARTICIPANT"
				+ "    WHERE COURSE_SCHEDULE_ID = CSCHED.ID" + ") AS PARTICIPANT_ID," + "CSCHED.STATUS AS STATUS,"
				+ "CSCHEDDET.SCHEDULED_START_DATETIME AS SCHEDULED_START_DATETIME,"
				+ "CSCHEDDET.SCHEDULED_END_DATETIME AS SCHEDULED_END_DATETIME " + "FROM COURSE_SCHEDULE AS CSCHED "
				+ "INNER JOIN COURSE_SCHEDULE_DETAIL AS CSCHEDDET " + "ON CSCHED.ID = CSCHEDDET.COURSE_SCHEDULE_ID "
				+ "INNER JOIN COURSE AS C " + "ON CSCHED.COURSE_ID = C.ID " + "INNER JOIN EMPLOYEE AS E "
				+ "ON CSCHED.INSTRUCTOR_ID = E.ID " + "INNER JOIN VENUE AS V " + "ON CSCHED.VENUE_ID = V.ID "
				+ "LEFT JOIN COURSE_PARTICIPANT AS CPART " + "ON CSCHED.ID = CPART.COURSE_SCHEDULE_ID "
	...
	public Set<CourseParticipant> findCourseScheduleById(Long id) {
		String sql = "SELECT " + "CSCHED.ID AS ID, " + "CSCHEDDET.COURSE_SCHEDULE_ID AS COURSE_SCHEDULE_ID, "
				+ "C.NAME AS COURSE_NAME, " + "E.LAST_NAME AS INSTRUCTOR_LAST_NAME,"
				+ "E.FIRST_NAME AS INSTRUCTOR_FIRST_NAME, " + "V.NAME AS VENUE_NAME, "
				+ "CPART.PARTICIPANT_ID AS PARTICIPANT_ID, " + "(" + "SELECT LAST_NAME " + "    FROM EMPLOYEE "
				+ "    WHERE ID = CPART.PARTICIPANT_ID" + ") AS PARTICIPANT_LAST_NAME, " + "("
				+ "    SELECT FIRST_NAME " + "    FROM EMPLOYEE " + "    WHERE ID = CPART.PARTICIPANT_ID"
				+ ") AS PARTICIPANT_FIRST_NAME, " + "CSCHEDDET.SCHEDULED_START_DATETIME AS SCHEDULED_START_DATETIME, "
				+ "CSCHEDDET.SCHEDULED_END_DATETIME AS SCHEDULED_END_DATETIME, " + "CSCHEDDET.DURATION AS DURATION, "
				+ "E.EMAIL_ADDRESS AS EMAIL, " + "E.NUMBER AS EMPLOYEE_NUMBER "
				...
				+ "ON CSCHED.COURSE_ID = C.ID " + "INNER JOIN EMPLOYEE AS E " + "ON CSCHED.INSTRUCTOR_ID = E.ID "
				+ "INNER JOIN VENUE AS V " + "ON CSCHED.VENUE_ID = V.ID " + "INNER JOIN COURSE_PARTICIPANT AS CPART "
				+ "ON CSCHED.ID = CPART.COURSE_SCHEDULE_ID " + "WHERE CSCHED.ID = :id AND CSCHED.STATUS = 'A'";

		SqlParameterSource namedParameters = new MapSqlParameterSource().addValue("id", id);

		List<CourseParticipant> courseList = template.query(sql, namedParameters, new CourseParticipantRowMapper());
	...
	public Set<CourseAttendance> findCourseScheduleDetailParticipantsById(Long id) {
		String sql = "SELECT " + "CSCHED.ID AS ID, " + "CSCHEDDET.COURSE_SCHEDULE_ID AS COURSE_SCHEDULE_ID, "
				...
				+ "    WHERE ID = CPART.PARTICIPANT_ID" + ") AS PARTICIPANT_LAST_NAME, " + "("
				+ "    SELECT FIRST_NAME " + "    FROM EMPLOYEE " + "    WHERE ID = CPART.PARTICIPANT_ID"
				+ ") AS PARTICIPANT_FIRST_NAME, " + "CSCHED.STATUS AS STATUS, " + "CSCHEDDET.DURATION AS DURATION, "
				+ "CSCHEDDET.SCHEDULED_START_DATETIME AS SCHEDULED_START_DATETIME, "
				...
				+ "ON CSCHED.COURSE_ID = C.ID " + "INNER JOIN EMPLOYEE AS E " + "ON CSCHED.INSTRUCTOR_ID = E.ID "
				+ "INNER JOIN VENUE AS V " + "ON CSCHED.VENUE_ID = V.ID " + "INNER JOIN COURSE_PARTICIPANT AS CPART "
				+ "ON CSCHED.ID = CPART.COURSE_SCHEDULE_ID " + "INNER JOIN COURSE_ATTENDANCE AS CATTEN "
	...
	public Set<CourseAttendance> findCourseAttendanceByCourseScheduleDetailId(Long id) {
		String sql = "SELECT " + "CSCHED.ID AS ID,  " + "CSCHEDDET.COURSE_SCHEDULE_ID AS COURSE_SCHEDULE_ID, "
				...
				+ "V.NAME AS VENUE_NAME, " + "CATTEN.PARTICIPANT_ID AS PARTICIPANT_ID, " + "(" + "SELECT LAST_NAME "
				+ "    FROM tsup.EMPLOYEE  " + "    WHERE ID = CATTEN.PARTICIPANT_ID" + ") AS PARTICIPANT_LAST_NAME, "
				+ "(" + "    SELECT FIRST_NAME " + "    FROM tsup.EMPLOYEE " + "    WHERE ID = CATTEN.PARTICIPANT_ID"
				+ ") AS PARTICIPANT_FIRST_NAME, " + "CSCHEDDET.DURATION AS DURATION, " + "CSCHED.STATUS AS STATUS, "
				+ "CSCHEDDET.SCHEDULED_START_DATETIME AS SCHEDULED_START_DATETIME, "
				+ "CSCHEDDET.SCHEDULED_END_DATETIME AS SCHEDULED_END_DATETIME, "
				+ "CATTEN.LOG_IN_DATETIME AS LOG_IN_DATETIME " + "FROM COURSE_SCHEDULE AS CSCHED  "
				+ "INNER JOIN COURSE_SCHEDULE_DETAIL AS CSCHEDDET " + " ON CSCHED.ID = CSCHEDDET.COURSE_SCHEDULE_ID  "
				+ "INNER JOIN COURSE AS C " + "ON CSCHED.COURSE_ID = C.ID   " + "INNER JOIN EMPLOYEE AS E  "
				+ "ON CSCHED.INSTRUCTOR_ID = E.ID " + "INNER JOIN VENUE AS V  " + "ON CSCHED.VENUE_ID = V.ID "
				+ "INNER JOIN COURSE_ATTENDANCE AS CATTEN  " + "ON CSCHED.ID = CATTEN.COURSE_SCHEDULE_DETAIL_ID "
				+ "WHERE COURSE_SCHEDULE_DETAIL_ID = :id " + "AND CSCHED.STATUS = 'A'";
	...
	public void saveAttendance(CourseAttendance courseAttendance) {
		String sql = "INSERT INTO " + "COURSE_ATTENDANCE(COURSE_SCHEDULE_DETAIL_ID, PARTICIPANT_ID, "
		...
		SqlParameterSource namedParameters = new MapSqlParameterSource()
				...
				.addValue("status", courseAttendance.getStatus()).addValue("loginDateTime",
						courseAttendance.getLoginDateTime().withZoneSameInstant(ZoneId.of("UTC")).toOffsetDateTime());
	...
	public void updateAttendance(CourseAttendance courseAttendance) {
		String sql = "UPDATE COURSE_ATTENDANCE SET course_schedule_detail_id = :courseScheduleDetailId, "
				...
				+ "email = 'r.ramos@fujitsu.com' " + "WHERE id = :id ";

		SqlParameterSource namedParameters = new MapSqlParameterSource().addValue("id", courseAttendance.getId())
				.addValue("courseScheduleDetailId", courseAttendance.getCourseScheduleDetailId())
				.addValue("participantId", courseAttendance.getParticipantId())
				.addValue("status", courseAttendance.getStatus()).addValue("loginDateTime",
						courseAttendance.getLoginDateTime().withZoneSameInstant(ZoneId.of("UTC")).toOffsetDateTime());
	...
	public Set<CourseAttendance> findCourseScheduleDetailById(Long id) {
		...
		List<CourseAttendance> attendanceList = template.query(sql, namedParameters, new CourseAttendanceRowMapper());
